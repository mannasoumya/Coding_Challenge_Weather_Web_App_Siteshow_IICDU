  ...
  $.getJSON("http://gd.geobytes.com/GetCityDetails?callback=?", function(data) {
    ...
    $.getJSON(
      "http://api.ipstack.com/" +
        curr_ip_data +
        "?access_key=795b81a0c1ee039639d33b6e46f101f3",
      function(data) {
        map_img = createImg(
          "https://dev.virtualearth.net/REST/V1/Imagery/Map/Road/" +
            ...
            "%2C" +
            data.longitude +
            "/13?mapSize=500,250&format=png&pushpin=" +
            data.latitude +
            "," +
            data.longitude +
            ";64;Hi&key=AqB0MpU-01rhZ-j_isOHf_fad-KN7EFYtospc-vBpnwwVtZxbYFVqH3rviMasWAQ"
        );
        image(map_img, 0, 0);
          ...
          function(weather_raw) {
            ...
            function changebutton() {
              ...
              if (cel_b == true) {
                temperature.html(
                  "Temperature : " +
                    celciustofahren(weather_raw.main.temp) +
                    "° F"
                );
                ...
                temperature.html(
                  "Temperature : " + fahrentocelcius(temp_fah) + "° C"
                );
                switch_to_fahr.html("Switch To Fahrenheit");
                cel_b = true;
              }
            } //end of changebutton
            let pressure = createDiv(),
              humidity = createDiv();
            ...
            if (weather_raw.main.pressure !== undefined) {
              pressure.html(
                "Pressure : " + weather_raw.main.pressure + " milliBars"
              );
            } else {
            ...
            function searchweather() {
              ...
              $.getJSON(
                "https://openweathermap.org/data/2.5/weather?q=" +
                  searchbox.value() +
                  "&appid=b6907d289e10d714a6e88b30761fae22",
                function(search_weather_raw) {
                  temperature.html(
                    "Temperature : " + search_weather_raw.main.temp + "° C"
                  );

                  map_img = createImg(
                    "https://dev.virtualearth.net/REST/V1/Imagery/Map/Road/" +
                      search_weather_raw.coord.lat +
                      "%2C" +
                      search_weather_raw.coord.lon +
                      "/13?mapSize=500,250&format=png&pushpin=" +
                      search_weather_raw.coord.lat +
                      "," +
                      search_weather_raw.coord.lon +
                  ...
                  if (search_weather_raw.main.pressure !== undefined) {
                    pressure.html(
                      "Pressure : " +
                        search_weather_raw.main.pressure +
                        " milliBars"
                    );
                  } else {
                    pressure.html(
                      "Pressure : not available currently from API"
                    );
                  }
                  if (search_weather_raw.main.humidity !== undefined) {
                    humidity.html(
                      "Humidity : " + search_weather_raw.main.humidity + "%"
                    );
                  } else {
                    humidity.html(
                      "Humidity : not available currently from API"
                    );
                  }
                  switch_to_fahr2.mousePressed(changebutton2);
                  let cel_b2 = true;
                  function changebutton2() {
                    let temp_fah2 = celciustofahren(
                      search_weather_raw.main.temp
                    );
                    if (cel_b2 == true) {
                      temperature.html(
                        "Temperature : " +
                          celciustofahren(search_weather_raw.main.temp) +
                          "° F"
                      );
                      ...
                      temperature.html(
                        "Temperature : " + fahrentocelcius(temp_fah2) + "° C"
                      );
